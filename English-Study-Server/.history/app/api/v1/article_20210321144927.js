...
import { AddArticleValidator, DeleteArticleValidator, EditArticleValidator,ArticleSearchValidator } from '../../validator/article';
...
contentApi.get('/', async ctx => {
  ...
  if (cacheResult) {
    ...
    // 如果不存在，直接从数据库读取
    articleList = await ArticleDao.getArticleList(page, size);
 ...
 * 关键词查询
 */
 contentApi.get('/search/one', async ctx => {
  const v = await new BookSearchValidator().validate(ctx);
  const articleList = await ArticleDao.getArticleByKeyword(v.get('query.q'));
