...
class Comment {
  ...
  static getArticleComment(article_id, parent_comment_id = -1) {

    this.getCommentById(article_id, parent_comment_id).then(commentList => {

      commentList.forEach(commentItem => {
        this.getCommentById(article_id, commentItem.id).then(comment => {
