  ...
  methods: {
    ...
    async getArticles() {
      try {
        ...
        // 处理布尔值显示
        this.tableData.show_is_published = Number(this.tableData.is_published)
        this.tableData.show_is_comment_enabled = Number(this.tableData.is_comment_enabled)
        this.tableData.show_is_top = Number(this.tableData.is_top)
      } catch (error) {
        // 资源不存在
        if (error.code === 10020) {
          this.tableData = []
